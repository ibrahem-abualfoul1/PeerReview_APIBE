...
public class AnswersController : ControllerBase
{
    ...
    [HttpGet("mine")] public async Task<ActionResult<IEnumerable<Answer>>> Mine() => await _db.Answers.Where(a=>a.UserId==CurrentUserId).ToListAsync();

    [HttpPost] public async Task<ActionResult> Create(AnswerCreateDto dto)
    {
        var a = await _db.Answers.FirstOrDefaultAsync(x=>x.UserId==CurrentUserId && x.QuestionId==dto.QuestionId && x.QuestionItemId==dto.QuestionItemId);
    ...
    [HttpPut("{id:int}")] public async Task<ActionResult> Update(int id, AnswerUpdateDto dto){ var a=await _db.Answers.FindAsync(id); if (a==null || a.UserId!=CurrentUserId) return NotFound(); a.Value=dto.Value; a.SubmittedAt=DateTime.UtcNow; await _db.SaveChangesAsync(); return NoContent(); }

    [HttpDelete("{id:int}")] public async Task<ActionResult> Delete(int id){ var a=await _db.Answers.FindAsync(id); if(a==null || a.UserId!=CurrentUserId) return NotFound(); a.IsDeleted=true; a.DeletedAt=DateTime.UtcNow; await _db.SaveChangesAsync(); return NoContent(); }
