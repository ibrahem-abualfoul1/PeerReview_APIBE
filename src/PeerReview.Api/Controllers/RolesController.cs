...
public class RolesController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<ActionResult> Create(Role r)
    {
        _db.Roles.Add(r);
    ...
    [HttpPut("{id:int}")]
    public async Task<ActionResult> Update(int id, Role r)
    {
        var e = await _db.Roles.FindAsync(id);
        if (e == null) return NotFound();
        e.Name = r.Name;

        e.CanSeeAllUsers = r.CanSeeAllUsers;
        e.CanSeeSystemStats = r.CanSeeSystemStats;
        e.CanSeeAssignmentsAll = r.CanSeeAssignmentsAll;
        e.CanSeeAnswersAll = r.CanSeeAnswersAll;
        await _db.SaveChangesAsync();
        return NoContent();
    }
}
